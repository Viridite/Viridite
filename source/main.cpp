...
// ─── Material 3 motion ──────────────────────────────────────────────────────
// Android does not move things linearly and it does not move them with a plain
// exponential ease either. M3 specifies an *emphasized* curve — slow to start,
// fast through the middle, and a long settle — and that asymmetry is most of
// what makes a list feel like Android rather than like a menu. Durations come
// from the spec too: 500ms for something crossing the screen, 200ms for a
// small state change.
//
// cubic-bezier(0.2, 0.0, 0.0, 1.0), solved by bisection on x. Ten iterations
// is well past display precision and this runs a handful of times per frame.
static float cubicBezierY(float x, float x1, float y1, float x2, float y2) {
    if (x <= 0.0f) return 0.0f;
    if (x >= 1.0f) return 1.0f;
    float lo = 0.0f, hi = 1.0f, t = x;
    for (int i = 0; i < 10; i++) {
        float mt = 1 - t;
        float bx = 3*mt*mt*t*x1 + 3*mt*t*t*x2 + t*t*t;
...
static float m3Emphasized(float p)         { return cubicBezierY(p, 0.20f, 0.0f, 0.0f, 1.0f); }
static float m3EmphasizedDecel(float p)    { return cubicBezierY(p, 0.05f, 0.7f, 0.1f, 1.0f); }
static const Uint32 M3_DUR_SHORT = 200;    // small state changes
static const Uint32 M3_DUR_LONG  = 500;    // something crossing the screen

// Sticks and the hat, in one place.
//
// actionFor covers buttons and keys, but a stick is continuous: it needs rate
// limiting or a nudge scrolls to the end of the list. That logic used to live
// only in the app list, so every other screen quietly ignored the sticks —
// the launch picker, Manage, the self-test. Sharing one implementation is the
// only way "every screen answers to every input" stays true as screens are
// added, rather than being four copies that drift.
static Act motionAction(const SDL_Event& ev, Uint32& lastStick) {
    if (ev.type == SDL_JOYHATMOTION) {
        if (ev.jhat.value & SDL_HAT_DOWN)  return Act::Down;
        if (ev.jhat.value & SDL_HAT_UP)    return Act::Up;
        if (ev.jhat.value & SDL_HAT_LEFT)  return Act::PageUp;
        if (ev.jhat.value & SDL_HAT_RIGHT) return Act::PageDown;
        return Act::None;
    }
    if (ev.type != SDL_JOYAXISMOTION) return Act::None;
    Uint32 now = SDL_GetTicks();
    if (now - lastStick <= 180) return Act::None;   // readable stepping pace
    int a = ev.jaxis.axis, v = ev.jaxis.value;
    if      ((a == AXIS_LY || a == AXIS_RY) && v >  AXIS_DEADZONE) { lastStick = now; return Act::Down; }
    else if ((a == AXIS_LY || a == AXIS_RY) && v < -AXIS_DEADZONE) { lastStick = now; return Act::Up; }
    else if ((a == AXIS_LX || a == AXIS_RX) && v >  AXIS_DEADZONE) { lastStick = now; return Act::PageDown; }
    else if ((a == AXIS_LX || a == AXIS_RX) && v < -AXIS_DEADZONE) { lastStick = now; return Act::PageUp; }
    return Act::None;
}
...
struct App {
    ...
    float  selAnimY = -1.0f;
    // Scroll is animated separately from selection. Android slides the list
    // under the selection; snapping a whole row at a time is the thing that
    // most gives away a hand-rolled list.
    float  scrollAnim   = 0.0f;
    int    scrollAnimTo = 0;
    float  scrollFrom   = 0.0f;
    Uint32 scrollAt     = 0;
    float  selAnimFrom = 0.0f;
    int    selAnimTo = -1;
    Uint32 selAnimAt = 0;
    ...
    void render() {
        ...
        if (apks.empty()) {
            ...
            // Selection travel on the emphasized curve. A fixed-rate lerp
            // (0.35 of the remaining distance each frame) is an exponential
            // ease — it starts fast and crawls to a stop, which is the
            // opposite shape to Android's and reads as sluggish at the end.
            int targetY = LIST_Y + (int)(selected * ITEM_H - scrollAnim * ITEM_H);
            if (selAnimY < 0) { selAnimY = (float)targetY; selAnimFrom = (float)targetY; selAnimAt = 0; }
            if (targetY != selAnimTo) { selAnimFrom = selAnimY; selAnimTo = targetY; selAnimAt = now; }
            if (selAnimAt) {
                float p = (now - selAnimAt) / (float)M3_DUR_SHORT;
                if (p >= 1.0f) { selAnimY = (float)selAnimTo; selAnimAt = 0; }
                else selAnimY = selAnimFrom + (selAnimTo - selAnimFrom) * m3Emphasized(p);
            }
            {
            ...
            if (scroll != scrollAnimTo) { scrollFrom = scrollAnim; scrollAnimTo = scroll; scrollAt = now; }
            if (scrollAt) {
                float p = (now - scrollAt) / (float)M3_DUR_SHORT;
                if (p >= 1.0f) { scrollAnim = (float)scrollAnimTo; scrollAt = 0; }
                else scrollAnim = scrollFrom + (scrollAnimTo - scrollFrom) * m3Emphasized(p);
            }
            const float scrollPx = scrollAnim * ITEM_H;

            // One row of overscan each way so a row sliding in is drawn before
            // it reaches the edge, instead of appearing at it.
            int first = std::max(0, (int)scrollAnim - 1);
            int end   = std::min((int)apks.size(), (int)scrollAnim + VISIBLE + 2);
            for (int i = first; i < end; i++) {
                int iy = LIST_Y + (int)(i * ITEM_H - scrollPx);
    ...
    void showSelfTest() {
        ...
        while (!done) {
            ...
            while (SDL_PollEvent(&ev)) {
                ...
                {
                    static Uint32 stStick = 0;
                    switch (motionAction(ev, stStick)) {
                        case Act::Down: scroll = std::min(std::max(0,(int)res.size()-VIS), scroll+1); break;
                        case Act::Up:   scroll = std::max(0, scroll - 1); break;
                        default: break;
                    }
    ...
    void showThemes() {
        ...
        while (!done) {
            ...
            while (SDL_PollEvent(&ev)) {
                if (ev.type == SDL_QUIT) { done = true; break; }
                static Uint32 thStick = 0;
                Act ta = actionFor(ev);
                { Act m = motionAction(ev, thStick); if (m != Act::None) ta = m; }
                switch (ta) {
    ...
    void showManage() {
        ...
        while (!done) {
            ...
            while (SDL_PollEvent(&ev)) {
                ...
                Act a = actionFor(ev);
                { Act mm = motionAction(ev, lastManageStick); if (mm != Act::None) a = mm; }
    ...
    bool askLaunchMode(const std::string& appName,
        ...
        while (!done) {
            ...
            while (SDL_PollEvent(&ev)) {
                ...
                Act a = actionFor(ev);
                {
                    Act m = motionAction(ev, pickStick);
                    if (m != Act::None) a = m;
                }
